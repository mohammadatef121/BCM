...
ERROR_STATUS UART_Init(UART_cfg_s *pUART_cfg_s)
{
	...
	ASSIGN_BIT(u8_UCSRC_Buffer,STOP_MASK,((pUART_cfg_s->u8_StopBit) << STOP_BITS_OFFSET));
	ASSIGN_BIT(u8_UCSRC_Buffer,FRAME_MASK,(pUART_cfg_s->u8_DataSize) )<<(BITS_PER_FRAME_OFFSET);
	ASSIGN_BIT(u8_UCSRC_Buffer,PARITY_MASK,(pUART_cfg_s->u8_ParityBit<<PARITY_BITS_OFFSET));
	UART_CSRC = u8_UCSRC_Buffer;
